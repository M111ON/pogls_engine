...
#include "geo_shatter.h"   /* S7: ShatShard, shat_make_shard() — data[5..7] */
...
typedef struct {
    ...
    /* S7: ORB Shatter context (not owned — pointer to PipelineWire's shat)
     * NULL = no shatter integration (backward compat)                   */
    ShatCtx           *shat;
...
static inline uint32_t detach_flush_pass(DetachLane *dl)
{
    ...
    for (uint32_t i = 0; i < avail; i++) {
        ...
        /* S7: data[5..7] = ShatShard — ORB Shatter context
         * Two worlds mount:
         *   Face world   → core.raw engine_id → EngineSlice
         *   Shadow world → GiantShadow lane 53 (this lane)
         * ShatShard fits exactly in 3×uint64_t (24B)                   */
        if (dl->shat && shat_in_shatter(dl->shat)) {
            ShatShard sh = shat_make_shard(dl->shat,
                                           (uint8_t)(e->phase18 % 6u));
            /* pack ShatShard into data[5..7]
             * [5]: shard_id (64b)
             * [6]: spoke(8) | invert_spoke(8) | stage(8) | anomaly_cyc(32)
             * [7]: anchor_op_count (64b)                                */
            blk[i].data[5] = sh.shard_id;
            blk[i].data[6] = ((uint64_t)sh.spoke)
                           | ((uint64_t)sh.invert_spoke    << 8)
                           | ((uint64_t)sh.stage_at_shatter << 16)
                           | ((uint64_t)sh.anomaly_cycles  << 32);
