...
/* World B lane id → index 0-251 (S7: expanded from 4→252) */
static inline int _b_idx(uint8_t lane_id) {
    return (int)(lane_id - LANE_B_START);   /* 4→0 .. 255→251 */
}

/* S7: Dynamic filename — lane_b{N}.delta / lane_b{N}.pending
 * Legacy lanes 4-7 keep original names for backward compat          */
static void _b_delta_name(uint8_t lane_id, char *out, size_t sz) {
    switch (lane_id) {
    case LANE_B_X:  snprintf(out, sz, "%s", FNAME_B_LANE_X);  break;
    case LANE_B_NX: snprintf(out, sz, "%s", FNAME_B_LANE_NX); break;
    case LANE_B_Y:  snprintf(out, sz, "%s", FNAME_B_LANE_Y);  break;
    case LANE_B_NY: snprintf(out, sz, "%s", FNAME_B_LANE_NY); break;
    default: snprintf(out, sz, "lane_b%u.delta", (unsigned)lane_id); break;
    }
}
static void _b_pending_name(uint8_t lane_id, char *out, size_t sz) {
    switch (lane_id) {
    case LANE_B_X:  snprintf(out, sz, "%s", FNAME_B_PENDING_X);  break;
    case LANE_B_NX: snprintf(out, sz, "%s", FNAME_B_PENDING_NX); break;
    case LANE_B_Y:  snprintf(out, sz, "%s", FNAME_B_PENDING_Y);  break;
    case LANE_B_NY: snprintf(out, sz, "%s", FNAME_B_PENDING_NY); break;
    default: snprintf(out, sz, "lane_b%u.pending", (unsigned)lane_id); break;
    }
}

static void _b_pending_path(const Delta_ContextB *ctx, int idx, char *out, size_t sz) {
    char name[64];
    _b_pending_name((uint8_t)(idx + LANE_B_START), name, sizeof(name));
    snprintf(out, sz, "%s/%s", ctx->pogls_dir_b, name);
}
static void _b_delta_path(const Delta_ContextB *ctx, int idx, char *out, size_t sz) {
    char name[64];
    _b_delta_name((uint8_t)(idx + LANE_B_START), name, sizeof(name));
    snprintf(out, sz, "%s/%s", ctx->pogls_dir_b, name);
}
...
int delta_b_audit(const Delta_ContextB *ctx) {
    ...
    /* Legacy pair check: lanes 4-7 (X/NX, Y/NY) — unchanged */
    ...
    /* S7: spoke-pair audit for expanded lanes 8-251
     * Each spoke group (6 lanes) audits via LANE_B_SPOKE mapping
     * Invariant: seq drift within same spoke group ≤ 1              */
    for (int i = 4; i < (int)LANE_B_COUNT - 5; i += 6) {
        int64_t base_seq = (int64_t)ctx->lane_seq[i];
        for (int j = 1; j < 6 && (i + j) < (int)LANE_B_COUNT; j++) {
            int64_t d = base_seq - (int64_t)ctx->lane_seq[i + j];
            if (d < -1 || d > 1) {
                fprintf(stderr,
                    "[delta_b] spoke-audit FAIL  group=%d lane_idx=%d drift=%lld\n",
                    i / 6, i + j, (long long)d);
                return -1;
            }
        }
...
Delta_DualRecovery delta_ab_recover(const char *source_path) {
    ...
    if (result.world_a == DELTA_RECOVERY_TORN) {
        ...
        int all_empty = 1;
        for (int i = 0; i < 4; i++) {   /* World A lanes only — 4 fixed */
