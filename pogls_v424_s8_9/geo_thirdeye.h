...
/* ── S7: 145-Drift secondary signal (144²=2⁸×3⁴ insight) ────────── *
 * Primary:   imbalance/hot → ANOMALY (unchanged)                      *
 * Secondary: op_count%144 == 1 → soft drift hint (+1 off-center)      *
 *            not a fail — layer on top, never replaces primary         */
#define TE_DRIFT_CYCLE   144u   /* sync period = GEO_TE_CYCLE          */
#define TE_DRIFT_OFFSET  1u     /* +1 drift from perfect symmetry      */
#define TE_DRIFT_FLAG    0x01u  /* bit in drift_state                  */
...
typedef struct {
    ...
    uint8_t  drift_state;       /* S7: TE_DRIFT_FLAG set = soft drift  */
} ThirdEye;
...
static inline void te_tick(ThirdEye *te, GeoSeed cur, uint8_t spoke, uint8_t slot_hot) {
    ...
    /* S7: 145-drift — secondary soft signal (layer on top, not replace)
     * op_count%144==1 means we're 1 tick past a sync boundary → drift */
    te->drift_state = ((te->op_count % TE_DRIFT_CYCLE) == TE_DRIFT_OFFSET)
                      ? TE_DRIFT_FLAG : 0u;
...
/* S7: drift query — non-zero = soft drift signal active this tick */
static inline uint8_t te_is_drift(const ThirdEye *te) {
    return te->drift_state & TE_DRIFT_FLAG;
}
...
static inline void te_status(const ThirdEye *te) {
    ...
    printf("  QRPN state : %s%s\n", te_state_name(te->qrpn_state),
           te_is_drift(te) ? "  [DRIFT+1]" : "");
