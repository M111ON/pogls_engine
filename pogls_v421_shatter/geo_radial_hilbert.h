...
/* ── S7/P3: Domain XOR — derive expected_xor from geometry constants ─ *
 *                                                                        *
 * Formula (locked S6, no magic numbers):                                *
 *   base    = GEO_FACE_UNITS  (64)                                      *
 *   face_c  = face  × GEO_GROUP_SIZE   (face  ∈ 0..8)                  *
 *   group_c = group × GEO_SPOKES       (group ∈ 0..7)                  *
 *   xor     = ((base + face_c + group_c) & 0xFF) ^ (face × group)      *
 *                                                                        *
 * Update point: when real domain data arrives, change only this fn.     *
 * Everything downstream (rh_audit_group_domain) stays unchanged.        */
static inline uint8_t rh_domain_xor(uint8_t face, uint8_t group) {
    uint8_t base    = (uint8_t)GEO_FACE_UNITS;
    uint8_t face_c  = (uint8_t)(face  * GEO_GROUP_SIZE);
    uint8_t group_c = (uint8_t)(group * GEO_SPOKES);
    return (uint8_t)(((uint16_t)(base + face_c + group_c) & 0xFFu)
                     ^ (uint8_t)(face * group));
}

/* ── S7/P3: Domain-aware audit (replaces rh_audit_group with xor=0) ── *
 *                                                                        *
 * Use this instead of rh_audit_group(buf, spoke, 0).                   *
 * face  = addr.unit >> 6  (0..8, outer faces only)                     *
 * group = addr.unit >> 3  (0..7)                                       */
static inline RHAuditResult rh_audit_group_domain(
    const uint8_t buf[GEO_GROUP_SIZE],
    uint8_t spoke,
    uint8_t face,
    uint8_t group)
{
    return rh_audit_group(buf, spoke, rh_domain_xor(face, group));
}
